...
@time NeutronStarOscillations.FrequencyDomain.PerfectFluid.Matrix.get_eigensystem(PF_star, N1, h_TOV, N_eigvals);
@time NeutronStarOscillations.FrequencyDomain.PerfectFluid.Matrix.get_eigensystem(PF_star, N2, h_TOV, N_eigvals);
@time NeutronStarOscillations.FrequencyDomain.PerfectFluid.Matrix.get_eigensystem(PF_star, N3, h_TOV, N_eigvals);

# load eigensystems. Eigenvalues are linear frequencies squared, i.e., ν^2 where ν = ω / (2π) and [ν] = kHz
omega_squared_1, evecs_1, r_1 = NeutronStarOscillations.FrequencyDomain.PerfectFluid.Matrix.load_eigensystem(PF_star, N1);
omega_squared_2, evecs_2, r_2 = NeutronStarOscillations.FrequencyDomain.PerfectFluid.Matrix.load_eigensystem(PF_star, N2);
omega_squared_3, evecs_3, r_3 = NeutronStarOscillations.FrequencyDomain.PerfectFluid.Matrix.load_eigensystem(PF_star, N3);
isapprox(omega_squared_3, NeutronStarOscillations.FrequencyDomain.PerfectFluid.Matrix.get_eigenvalues(PF_star, N3, h_TOV, N_eigvals))
