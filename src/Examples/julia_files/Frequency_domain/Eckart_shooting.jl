...
@time NeutronStarOscillations.FrequencyDomain.Eckart.Shoot.get_eigensystem(Eckart_star, h1_shoot, h_TOV, N_eigvals; nPointsMatrix = N_matrix, print_progress = true);
@time NeutronStarOscillations.FrequencyDomain.Eckart.Shoot.get_eigensystem(Eckart_star, h2_shoot, h_TOV, N_eigvals; nPointsMatrix = N_matrix, print_progress = true);
@time NeutronStarOscillations.FrequencyDomain.Eckart.Shoot.get_eigensystem(Eckart_star, h3_shoot, h_TOV, N_eigvals; nPointsMatrix = N_matrix, print_progress = true);

# load eigensystems. Eigenvalues are (complex-valued) linear frequencies, i.e., ν = ω / (2π) and [ν] = kHz
init_freqs_1, shooting_freqs_1, radii_1, evecs_1, resids_1, ret_1 = NeutronStarOscillations.FrequencyDomain.Eckart.Shoot.load_eigensystem(Eckart_star, h1_shoot);
init_freqs_2, shooting_freqs_2, radii_2, evecs_2, resids_2, ret_2 = NeutronStarOscillations.FrequencyDomain.Eckart.Shoot.load_eigensystem(Eckart_star, h2_shoot);
init_freqs_3, shooting_freqs_3, radii_3, evecs_3, resids_3, ret_3 = NeutronStarOscillations.FrequencyDomain.Eckart.Shoot.load_eigensystem(Eckart_star, h3_shoot);
